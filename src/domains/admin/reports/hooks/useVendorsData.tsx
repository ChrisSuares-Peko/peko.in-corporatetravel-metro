...
import { getAllData, getFileBufferReport } from '../api/vendor';
import { getData } from '../types/index';
import { TransactionInfo, transactionResponse } from '../types/vendors';
    ...
    const downloadReport = async (type: string) => {
        ...
        if (data) {
            ...
            if (type === 'excel') {
                ...
                saveAs(blob, `Vendor Report.csv`);
            } else if (type === 'pdf') {
                saveAs(blob, `Vendor Report.pdf`);
            }
        }
        setIsLoading(false);
    };
