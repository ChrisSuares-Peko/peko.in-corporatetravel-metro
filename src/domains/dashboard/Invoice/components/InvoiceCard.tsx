...
const InvoiceCard = ({
    ...
    return (
        <Flex vertical gap={5} className="items-center justify-center w-full">
            <Card
                ref={componentRef}
                className="w-full p-3 border border-gray-300 rounded-md md:max-w-3xl"
            >
                <Row gutter={[0, 20]} className="w-full" align="top">
                    ...
                    >
                        <Flex vertical gap={5}>
                            <Text className="font-bold md:text-sm xs:text-[0.7rem]">From</Text>
                            {data && (
                                <Text className="font-medium xs:text-[0.7rem] md:text-sm">
                                    {data?.recipientDetails && data?.recipientDetails?.billerName}
                                </Text>
                            )}
                            {data && (
                                <Text className="xs:text-[0.7rem] md:text-sm">
                                    {data.recipientDetails &&
                                        data.recipientDetails?.billerCompanyAddress}
                                </Text>
                            )}
                            {data.recipientDetails?.billerPhone && (
                                <Text className="xs:text-[0.7rem] md:text-sm">
                                    {data.recipientDetails && data.recipientDetails?.billerPhone}
                                </Text>
                            )}
                            {data.recipientDetails?.billerEmail && (
                                <Text className="font-medium xs:text-[0.7rem] md:text-sm">
                                    {data.recipientDetails && data.recipientDetails?.billerEmail}
                                </Text>
                            )}
                            {data?.recipientDetails?.billerTRNNumber && (
                                <Text className="xs:text-[0.7rem] md:text-sm">
                                    {data?.recipientDetails?.billerTRNNumber}
                                </Text>
                            )}
                        ...
                        <Flex vertical gap={5}>
                            <Text className="font-bold xs:text-[0.7rem] md:text-sm ">
                                Billed to
                            </Text>
                            {data && (
                                <Text className="font-medium xs:text-[0.7rem] md:text-sm">
                            ...
                            {data?.recipientDetails?.customerTRNNumber && (
                                <Text className="xs:text-[0.7rem] md:text-sm">
                                    {data?.recipientDetails?.customerTRNNumber}
                                </Text>
                            )}
                    ...
                    <Col xs={9} md={8} className="xs:mt-4 md:mt-10">
                        <Flex vertical gap={10}>
                            <Flex gap={10} justify="space-between">
                                <Text className="font-normal xs:text-[0.7rem] md:text-sm">
                                    Invoice No
                                </Text>
                                {data && (
                                    <Text className="font-medium xs:text-[0.7rem] md:text-sm">
                    ...
                    <Flex className="justify-end w-2/3">
                        <Flex vertical className="overflow-scroll w-fit">
                            <Flex vertical gap={10} className="w-full">
                                ...
                                <Flex className="md:px-2">
                                    <Flex>
                                        <Text className="font-medium w-36 text-nowrap">Gst</Text>
                                    </Flex>
                                    ...
                                    <Flex className="flex-1">
                                        <Text className="w-32 font-semibold xs:text-xs md:text-sm text-nowrap">
                                            ...
                                            {parseFloat(data?.paymentDetails?.shipping)
                                                ? parseFloat(data.paymentDetails.shipping).toFixed(
                                                      2
                                                  )
                                                : '0.00'}
                                        </Text>
                                ...
                                <Flex justify="space-between" className="md:px-2">
                                    <Flex>
                                        <Text className="font-medium w-36 text-nowrap">Total</Text>
                                    </Flex>
                ...
                <Flex vertical gap={10} className="mt-2">
                    ...
                    {JSON.parse(data?.comments) && (
                        <Flex vertical>
                            <Text className="font-bold xs:text-[0.7rem] md:text-sm">Notes</Text>
