...
const TableMobile: React.FC<TableProp> = ({ data }) => {
    const { date, status, amount, serviceType, providerId } = data;
    ...
    const details = [
        ...
        { label: 'Tracking No', value: providerId },
    ];
    return (
                    ...
                    <Col xs={7}>
                        <Flex justify="center">
                            <Typography.Text className="text-xs font-normal text-center text-textDarkGray">
                                INR {formatNumberWithLocalString(parseFloat(amount))}
                            </Typography.Text>
                        </Flex>
                    </Col>
                    <Col xs={8}>
                    ...
                    <Flex vertical gap={10} className="p-6 bg-bgLightGray">
                        {details.map((detail, index) => (
                            <DetailSection
                                key={index}
                                label={detail.label}
                                value={detail.value ? detail.value : 'N/A'}
                            />
                        ))}
                        <Button
                            size="small"
