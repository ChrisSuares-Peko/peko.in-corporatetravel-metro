...
interface LoadFundsDrawerProps {
    open: boolean;
    onBack: () => void;
    onboardingRecord: {
...
const LoadFundsDrawer = ({ open, onBack, onboardingRecord, balance, balanceLoading, virtualAccountNumber, ifsc }: LoadFundsDrawerProps) => {
        ...
        >
            <Flex vertical gap={16}>
                ...
                >
                    ...
                    <Flex vertical gap={8}>
                        {(virtualAccountNumber ?? onboardingRecord?.virtualAccountNumber) && (
                            <DetailRow label="Account Number" value={(virtualAccountNumber ?? onboardingRecord?.virtualAccountNumber)!} />
                        )}
                        {(ifsc ?? onboardingRecord?.virtualIfsc) && (
                            <DetailRow label="IFSC" value={(ifsc ?? onboardingRecord?.virtualIfsc)!} />
                        )}
                    </Flex>
                </Flex>
                ...
                {isActive && (
                    <Flex gap={12}>
                        <Flex
                            vertical
                            gap={4}
                            style={{ flex: 1, background: '#F8FAFC', borderRadius: 10, padding: '10px 14px' }}
                        >
...
export default LoadFundsDrawer;
