...
const PDFViewer = ({
    ...
    const handlePageDrop = (e: React.DragEvent<HTMLDivElement>, pageIndex: number) => {
        ...
        const isOverlapping = (excludeId?: string) => signatureFields.some(
            sf =>
                sf.id !== excludeId &&
                sf.page === pageIndex &&
        ...
        if (data.type === 'existing' && data.fieldId) {
            if (isOverlapping(data.fieldId)) {
                dispatch(showToast({ description: 'Cannot drop here. This position has already been assigned for signing.', variant: 'error' }));
                return;
            }
            onSignatureFieldsChange?.(
    ...
    return (
        <Flex
            className="rounded-[18px] border border-[#E5E7EB] overflow-hidden h-[400px] md:h-[820px]"
            style={{ width: '100%' }}
        >
            {showPageSelector && (
                <Flex
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {signatureFields
                                            .filter(f => f.page === pi)
                                            .map(field => {
                                                const color = getSignerColor?.(field.signerIndex) || {
                                                    bg: '#D9EECC',
                                                    border: '#05BE63',
                                                    text: '#15803D',
                                                };
                                                        ...
                                                        {editable && (
                                                            <CloseCircleOutlined
                                                                ...
                                                                onClick={e => {
                                                                    e.stopPropagation();
                                                                    onSignatureFieldsChange?.(
                                                                        signatureFields.filter(f => f.id !== field.id)
                                                                    );
                                                                }}
                                                            />
                                                        )}
