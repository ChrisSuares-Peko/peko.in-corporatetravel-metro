...
const DetailCard = ({ icon, label, title, subText, action, className, children }: Props) => (
    <Flex
        ...
        className={twMerge('px-4 py-4 md:px-5 bg-gray-50 rounded-2xl border border-gray-200', className)}
    >
        <Flex
            align="center"
            gap={16}
        ...
        >
            ...
            <Flex vertical gap={2} className="min-w-0">
                ...
                {subText && (
                    <Typography.Text className="!text-gray-500 !text-xs break-all">{subText}</Typography.Text>
                )}
            </Flex>
        </Flex>
