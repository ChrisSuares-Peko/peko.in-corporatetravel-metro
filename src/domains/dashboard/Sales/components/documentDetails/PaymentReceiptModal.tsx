...
const methodLabel = (val: string) =>
    PAYMENT_METHODS.find(o => o.value === val)?.label ?? val;
...
const PaymentReceiptModal = ({
    ...
    const { documentNo, currency, customerName, customerEmail, customerPhone, billerName } = context;
    const receiptNo = payment.receiptNo || `RC-${documentNo}-${payment.id}`;
