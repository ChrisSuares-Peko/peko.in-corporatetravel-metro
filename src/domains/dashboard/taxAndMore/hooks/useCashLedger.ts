...
import { getCashLedger } from '../api/tax';
import { CashLedgerTransaction } from '../types';
...
const useCashLedger = (params: Params | null) => {
    const { id, role } = useAppSelector(state => state.reducer.auth);
    const [transactions, setTransactions] = useState<CashLedgerTransaction[]>([]);
    ...
    const fetch = useCallback(async () => {
        ...
        setIsLoading(true);
        const resp = await getCashLedger({
            userId: id,
            userType: role,
            gstin: params.gstin,
