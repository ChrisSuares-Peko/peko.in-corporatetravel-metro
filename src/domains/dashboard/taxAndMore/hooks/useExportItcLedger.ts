...
import { exportItcLedger } from '../api/tax';
...
const useExportItcLedger = () => {
    ...
    const exportCsv = async (params: Params) => {
        ...
        setIsExporting(true);
        try {
            const resp = await exportItcLedger({
                userId: id,
                userType: role,
                gstin: params.gstin,
            ...
            saveAs(blob, `ItcLedger_${params.gstin}_${params.financialYear}_${params.month}.csv`);
        } finally {
...
export default useExportItcLedger;
