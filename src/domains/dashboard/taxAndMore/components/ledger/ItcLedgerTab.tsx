...
import type { ItcLedgerTransaction, ReturnLiabilityData } from '../../types';
...
interface Props {
    ...
    onTaxHeadChange: (v: string | undefined) => void;
    onExport: () => void;
}

const COLS = '1.2fr 1.8fr 2.5fr 1fr 1.2fr 1.2fr';
const HEADERS = ['Date', 'Reference No.', 'Description', 'Type', 'Amount (₹)', 'Balance After (₹)'];
...
const ItcLedgerTab = ({
    ...
    <Flex vertical gap={16}>
        <div className="border border-[#e2e8f0] rounded-[14px] overflow-hidden bg-white px-4 sm:px-6 py-5">
            ...
            />
            <SummaryBar
                leftLabel="ITC Credited"
                leftValue={`+₹${fmt(credits)}`}
                rightLabel="ITC Utilised"
                ...
                {!isLoading &&
                    rows.map((row, i) => (
                        <div
                            ...
                            style={{ gridTemplateColumns: COLS, minHeight: 52, minWidth: 560 }}
                        >
