...
import FinancialDocMobileCard from '../../../components/Financials/AdaptiveView/FinancialDocMobileCard';
...
describe('FinancialDocMobileCard', () => {
    const mockItem = {
    ...
    it('renders correctly with the provided item data', () => {
        ...
        expect(screen.queryByText('7891011')).not.toBeInTheDocument();
    ...
    it('toggles details view on clicking "See More"', () => {
        ...
        expect(screen.queryByText(/Issued Date/i)).not.toBeInTheDocument();
    ...
    it('shows loading spinner on download button when loading', () => {
        const loadingRowsWithId = { '2': true };
        render(
            <FinancialDocMobileCard
                item={mockItem}
                handleDelete={handleDelete}
    ...
    it('calls formatDate correctly for issueDate and expireDate', () => {
        ...
        expect(formatDate).toHaveBeenCalledWith('2024-02-01');
        expect(formatDate).toHaveBeenCalledWith('2025-02-01');
    });
});
