...
const EmirateDocCard = ({
    ...
    const handleDocDownload = (record: any) => {
        handleDocumentDownload(record, 'Emirate ID');
    };
        ...
        <Row className="p-6 border-t border-b md:border-t-0 h-1/3" justify="space-between">
            <Col span={24} md={19} className="flex items-center gap-2">
                <Flex className="bg-white " align="center">
                    <Image width={45} preview={false} src={emiratesId} />
                </Flex>
                <Col>
                    <Typography.Text className="text-base font-medium">Emirates ID</Typography.Text>
                    <Row className="flex gap-2">
                        <Typography.Text className="text-xs">
                            Issued Date:{' '}
                            {(owner.emiratesId.issueDate &&
                                owner.emiratesId.issueDate.split('T')[0]) ||
                                'N/A'}
                        </Typography.Text>
                        <Typography.Text className="text-xs">
                            Expiry Date:{' '}
                            {(owner.emiratesId.expireDate &&
                                owner.emiratesId.expireDate.split('T')[0]) ||
                                'N/A'}
                        </Typography.Text>
                    </Row>
                    <Flex align="center" gap={2}>
                        <Typography.Text className="text-xs">
                            EID No.{owner.emiratesId?.documentNumber || 'N/A'}
                        </Typography.Text>
                        {/* clipboard icon */}
                        {owner.emiratesId?.documentNumber && (
                            <ReactSVG
                                src={clipBoard}
                                className="cursor-pointer"
                                onClick={() => {
                                    copyToClipboard(owner.emiratesId?.documentNumber);
            ...
            >
                <Flex vertical gap="small" align="center" justify="center">
                    ...
                    {owner.emiratesId?.document && (
                        // <a
                        //     href={owner.emiratesId?.document || ''}
                        //     target="_blank"
                        //     rel="noopener noreferrer"
                        //     download
                        // >
                        <Button
                            onClick={() => handleDocDownload(owner.emiratesId?.document)}
                            loading={isLoading}
